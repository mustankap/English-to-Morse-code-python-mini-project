...
c.execute(''' SELECT count(name) FROM sqlite_master WHERE type='table' AND name='con' ''')
...
my_label0.grid(row=0, column=0,columnspan=2)
# instruction to user
mylabel1 = Label(
    root, text="          ENTER YOUR SENTENCE (ENGLISH/MORSE) :", padx=60, pady=2)
mylabel1.grid(row=1,columnspan=2)
...
e1.grid(row=2, column=0, padx=15, pady=10,columnspan=2)


# button to open another window
ct = Button(root, text="SEE THE CONVERSION TABLE:",
                       command=tab, bg="#58BFF0", padx="80", pady="10")
ct.grid(row=5, column=0)

# convert button
cv = Button(root, text="CONVERT !",
                       command=convert, bg="#58BFF0", padx="5", pady="5")
cv.grid(row=2, column=2)


ref_but=Button(root,text="Refresh !",command=ref,bg="#58BFF0", padx="80", pady="10")
ref_but.grid(row=6,column=0,columnspan=2)

his = Button(root, text="Previous Conversions:",
                       command=showtodd, bg="#58BFF0", padx="80", pady="10")
his.grid(row=5, column=1)
#removeconnection
conn.close()
